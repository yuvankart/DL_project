...
from core.modules.diffusionmodules.model import Encoder, Decoder, RestrictedDecoder
...
class VQModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Calculate autoencoder loss and discriminator loss
        ...
        # Log only unique metrics
        rec_loss = log_dict_ae.pop("val/rec_loss", None)  # Remove from log_dict_ae to avoid duplicate logging
        ...
        # Log remaining metrics from the dictionaries
        self.log_dict(log_dict_ae)
        self.log_dict(log_dict_disc)

        # Optionally log images for debugging
        self.debug_log_image(x, batch_idx, tag='gt')
        self.debug_log_image(xrec, batch_idx)

        # Return the logged dictionaries (can be adjusted based on your requirements)
        return {"aeloss": aeloss, "rec_loss": rec_loss, "discloss": discloss}
