...
class Places(Dataset):
    ...
    def initialize_paths(self):
        ...
        for root, dirs, files in os.walk(self.dataroot):
            image_files = glob.glob(os.path.join(root, f"*.{self.ext}"))
            data_list += [(path, '_'.join(path.split('/')[-3:])) for path in image_files]
        
        # Keep only the first 1/4 of the dataset
        ...
        if self.maskroot is not None:
            ...
            for root, dirs, files in os.walk(self.maskroot):
                ...
                mask_list += image_files
            
            # Keep only the first 1/4 of the masks
            self.mask_list = mask_list[:len(mask_list) // 4]
        else:
            self.mask_list = None
    ...
    def __len__(self):
        # Return the length of the reduced dataset
