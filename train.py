...
from core.data.utils import custom_collate
import core.visualizer as V
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not "gpus" in trainer_config:
            ...
            if isinstance(trainer_config.gpus, str):
                ngpu = len(trainer_config.gpus.strip(",").split(','))
            elif isinstance(trainer_config.gpus, int):
                ngpu = trainer_config.gpus
            else:
                raise ValueError("Unexpected type for trainer_config.gpus")
        ...
        if not opt.no_test and not trainer.interrupted:
            trainer.test(model, data._test_dataloader())
    except Exception:
